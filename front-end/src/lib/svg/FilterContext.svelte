...
<script lang="ts" module>
	...
	const colorToId = (color: ShadowColor | RoundShadowColor | InsetShadowColor, type: FilterType): string => `${type}${color.substring(6)}Filter`
	const idToFilter = (id: string): string => `url(#${id})`

	export function getFilters({ shadow = [], insetShadow = [], roundShadow = [] }: Filters): string {
		if (!getContext(contextKey)) console.trace('filter used outside of FilterContext');
